...
def build_autoguidance_config_path(model_size):
    ...
    # model_size_map = {
    #     's': 'lightningdit_s_sdvae_f16d32_semvaebasech16kl1e-7_svnorm_semfirst_repabase_400k',
    #     'b': 'lightningdit_b_sdvae_f16d32_semvaebasech16kl1e-7_svnorm_semfirst_repabase_400k'
    # }

    # if model_size not in model_size_map:
    #     raise ValueError(f"Invalid model_size: {model_size}. Must be 's' or 'b'")

    # folder_name = model_size_map[model_size]
    # config_path = f'configs/ours/_autoguidance/{folder_name}/inference.yaml'
...
def do_sample(train_config, accelerator, ckpt_path=None, cfg_scale=None, cfg_interval_start=None, timestep_shift=None,
    ...
    if use_autoguidance:
        if autoguidance_model_size is None:
            ...
            if autoguidance_config_path:
                # Default: use config file (no grid search)
            ...
            autoguidance_model_size = autoguidance_model_size if isinstance(autoguidance_model_size, list) else [autoguidance_model_size]
            autoguidance_ckpt_iter = autoguidance_ckpt_iter if isinstance(autoguidance_ckpt_iter, list) else [autoguidance_ckpt_iter]
    else:
        autoguidance_model_size = [None]
        autoguidance_ckpt_iter = [None]
    ...
    if use_autoguidance:
        if cfg_scale_sem is None:
            cfg_scale_sem = [None]  # Will use cfg_scale by default
        else:
            cfg_scale_sem = cfg_scale_sem if isinstance(cfg_scale_sem, list) else [cfg_scale_sem]

        if cfg_scale_tex is None:
            cfg_scale_tex = [None]  # Will use cfg_scale by default
        else:
            cfg_scale_tex = cfg_scale_tex if isinstance(cfg_scale_tex, list) else [cfg_scale_tex]
    else:
        cfg_scale_sem = [None]
        cfg_scale_tex = [None]
    ...
    if num_sampling_steps is None:
        num_sampling_steps = [None]  # Will use config value
    else:
    ...
    #         })
    cfg_scale = cfg_scale if isinstance(cfg_scale, list) else [cfg_scale]
    cfg_interval_start = cfg_interval_start if isinstance(cfg_interval_start, list) else [cfg_interval_start]
    timestep_shift = timestep_shift if isinstance(timestep_shift, list) else [timestep_shift]

    params_list = []
    # for scale, interval_start in zip(cfg_scale, cfg_interval_start):
    for cfg_scale_tmp in cfg_scale:
        for cfg_interval_start_tmp in cfg_interval_start:
            for timestep_shift_tmp in timestep_shift:
                for ag_model_size in autoguidance_model_size:
                    for ag_ckpt_iter in autoguidance_ckpt_iter:
                        for cfg_sem in cfg_scale_sem:
                            for cfg_tex in cfg_scale_tex:
                                for num_steps in num_sampling_steps:
                                    params_list.append({
                                        'cfg_scale': cfg_scale_tmp,
                                        'cfg_interval_start': cfg_interval_start_tmp,
                                        'timestep_shift': timestep_shift_tmp,
                                        'autoguidance_model_size': ag_model_size,
                                        'autoguidance_ckpt_iter': ag_ckpt_iter,
                                        'cfg_scale_sem': cfg_sem,
                                        'cfg_scale_tex': cfg_tex,
    ...
    for param in params_list:
        ...
        cfg_interval_start = param['cfg_interval_start']
        timestep_shift = param['timestep_shift']
        ag_model_size = param['autoguidance_model_size']
        ag_ckpt_iter = param['autoguidance_ckpt_iter']
        cfg_sem = param['cfg_scale_sem']
        cfg_tex = param['cfg_scale_tex']
        num_steps_tmp = param['num_sampling_steps']

        # Override num_sampling_steps if specified
        if num_steps_tmp is not None:
            train_config['sample']['num_sampling_steps'] = num_steps_tmp

        # Determine effective cfg_scale_sem and cfg_scale_tex
        # If not specified, use cfg_scale
        effective_cfg_sem = cfg_sem if cfg_sem is not None else cfg_scale
        effective_cfg_tex = cfg_tex if cfg_tex is not None else cfg_scale
        ...
        if use_autoguidance and (cfg_sem is not None or cfg_tex is not None):
        ...
        if not os.path.exists(sample_folder_dir):
            ...
            if png_count > train_config['sample']['fid_num']:
                ...
                sample_folder_dirs.append(sample_folder_dir)
                continue
